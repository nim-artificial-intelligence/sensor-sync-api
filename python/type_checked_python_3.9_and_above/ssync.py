#!/usr/bin/env python
import requests
from typing import ClassVar, Tuple, Any, Union
...
class SensorSync:
    ...
    def _url_for(self, api: str, **kwargs: Any) -> str:
        url = SensorSync.APIS[api]
        d = {'ip': self.ip}
        d.update(kwargs)
        return url.format(**d)

    def _request_url(self, url: str) -> Tuple[int, str]:
        assert self.ip is not None
        r = requests.get(url)
        return r.status_code, r.text

    def activate(self, sensor: int, verify: bool = False) -> Tuple[bool, str]:
    ...
    def reset(self, sensor: int, verify: bool = False) -> Tuple[bool, str]:
    ...
    def reboot(self, verify: bool = False) -> Tuple[bool, str]:
    ...
    def write_ip(self, new_ip: str, verify: bool = False) -> Tuple[bool, str]:
    ...
    def write_mac(self, new_mac:str, verify:bool = False) -> Tuple[bool, str]:
...
def t(s:Union[bool, int], t:str) -> None:
    if not s:
        print('*************', end='')
        print(repr(t))
    print(f'status: {s}\n{t}')
